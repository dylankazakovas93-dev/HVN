...
DEFAULT_OUTPUT = ROOT / "outputs" / "stage_02_generation_3_atomic_pilot_v3"
RESERVED = (
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        prog="run_atomic_pilot_v3.py",
        description=(
            "Generation 3 Pilot V3 structural-only run. Builds profiles, "
    ...
    for position, current in enumerate(dates):
        ...
        for relationship in Relationship:
            ...
            for ratio in RATIOS:
                ...
                if True:
                    nodes, _ = classify_composite_nodes(volume_profile, tpo_by_index)
                    for node in nodes:
                        row = base | {
...
def main(argv=None) -> None:
    ...
    manifest = {
        "generation": "GENERATION_3_PILOT_V3_COMPOSITE_ACTIVITY",
        ...
        "reproduction_command": (
            f"PYTHONPATH=src python3 scripts/run_atomic_pilot_v3.py --year {args.year} "
