...
DEFAULT_OUTPUT = ROOT / "outputs" / "stage_02_generation_3_atomic_pilot_v2"
RESERVED = (
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        prog="run_atomic_pilot_v2.py",
        description=(
            "Generation 3 Pilot V2 structural-only run. Builds profiles, "
...
def build(bars, *, year, code_sha):
        ...
        for relationship in Relationship:
            ...
            for ratio in RATIOS:
                ...
                for prominence in PROMINENCES:
                    nodes, _ = classify_nodes(
                        volume_profile, tpo_by_index, prominence_threshold=prominence
                    )
                    for node in nodes:
                        row = base | {
                            "prominence_threshold": prominence,
                            "node_id": f"{node.node_id}-P{prominence}",
                            ...
                            "zone_volume_share": node.zone_volume_share,
                            "zone_tpo_share": node.zone_tpo_share,
                            "zone_volume_density_ratio": node.zone_volume_density_ratio,
                            "zone_tpo_density_ratio": node.zone_tpo_density_ratio,
                            "peak_volume_percentile": node.peak_volume_percentile,
    ...
    return profiles, bin_rows, nodes_out, poc_rows, va_rows, tpo_va_rows
...
def main(argv=None) -> None:
    ...
    profiles, bin_rows, nodes, poc_rows, va_rows, tpo_va_rows = build(
        bars, year=args.year, code_sha=code_sha
    )

    for name, rows, sort_by in (
        ("atomic_candidates", nodes, ("node_id",)),
    ...
    for name, rows in (
        ...
        ("bin_normalization_summary.csv", bin_rows),
    ...
    summarize(nodes, ("rejection_reason",), output, "control_summary.csv")
    ...
    reconciliation = [
        ...
        {"check": "broad_plateaus_excluded",
         "value": sum(1 for r in nodes if r["geometry"] == BROAD_HIGH_VOLUME_PLATEAU)},
    ...
    manifest = {
        "generation": "GENERATION_3_PILOT_V2_SESSION_NORMALIZED",
        ...
        "reproduction_command": (
            f"PYTHONPATH=src python3 scripts/run_atomic_pilot_v2.py --year {args.year} "
